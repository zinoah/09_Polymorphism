package edu.kh.poly.modle.service;

import edu.kh.poly.modle.vo.Car;
import edu.kh.poly.modle.vo.Spark;
import edu.kh.poly.modle.vo.Tesla;
...
public class PolyService {
	...
	public void ex1() {
		...
		Car car2 = new Tesla();
		// Tesla 객체를 참조하는 변수타임이 Car(부모)이기 때문에
		...
		tesla.setFuel("전기");
		tesla.setWheel(4);
		tesla.setBetteryCapacity(100000);
		...
		arr[0] = car; //Car주소 == Car객체 (Car 주소는 Car타입객체)
		// arr[0] Car 참조 변수
		
		arr[1] = car2; //Tesla 주소 == Tesla 객체
		// arr[1] Car 참조 변수
		
		arr[2] = car3; //Spark == Spark객체
		// arr[2] Car 참조 변수
		...
		// 			-> Car상속 클래스는 모두 getEngine()을 가지고 있다를 정의.
	...
	public void ex2() {
		...
		Tesla t = new Tesla("전기모터", "전기", 4, 100000);
											   //배터리용량
		Spark s = new Spark("경차 엔진", "휘발유", 4, 6.5);
												 //휘발유
		...
		// Car를 대상으로 처음부터 검사하면 모두 True에서 검사가 안된다..
		for(int i = 0; i < arr.length; i++) {
	...
	public void ex3() {
		...
		// 자식객체의 본래 필드, 메소드를 사용하게 된다.
		
		Car c1 = new Tesla("전기모터", "전기", 4, 50000);//업캐스팅 된 상태 
		...
		// C1이 참조하는게 Tesl라는걸 알기 대문에 Tesla로 강제형변환 가능.
		// Tesla = Tesla 같은 자료형 연산 가능하니 같은 주로르 복사시켜버림.(얕은복사)
	...
	public void ex4() {
		...
		// -> java.lang.ClassCastExeption (형변환 예외)
	...
	public void ex5() {
		//바인딩 (Binding)
		// - 실제 실행할 메소드 코드와 호출하는 코드르 연결 시키는 것
		
		Car c1 = new Car("경유 엔진", "경유", 8);
		
		System.out.println(c1.getEngine()); // 
		// Car 객체에 있는 getEngine() 메소드를 호출 == 바인딩
		// String edu.kh.poly.modle.vo.Car.getEngine()
		
		// 프로그램 " 실행 전 "
		// - 컴파일러는 getEngine() 메소드가 Car에 있는 걸로 인식해서
		// c1.getEngine() 호출 코드와
		// String edu.kh.poly.modle.vo.Car.getEngine() 메소드 코드를 연결
		// -- > [정적 바인딩]
		
		System.out.println( c1.toString());
		// String edu.kh.poly.modle.vo.Car.toString()
		// Car 참조변수 c1을 이용해서
		// Car 객체에 잇는 오버라이딩 된 toString() 메소드르 호출
		...
		// String edu.kh.poly.modle.vo.Car.toString()
		// 참조변수 c2가 Car타입이므로
		// toString()도 car의 toString()를 호출 - 정적바인딩
		...
		for(int i = 0; i < arr.length; i++) {
			...
			//실행 전 : String edu.kh.poly.modle.vo.Car.toString()
			//실행 후 : 각 객체의 오버라이딩 된 toString()이 호출된 - 동적 바인딩
		}
